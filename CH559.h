...
#define bBOOT_LOAD                                                                                                     \
    0x20        // ReadOnly: boot loader status for discriminating BootLoader or Application: set 1 by power on reset,
                // clear 0 by software reset
#define bSW_RESET 0x10        // software reset bit, auto clear by hardware
#define bCODE_WE                                                                                                       \
    0x08        // enable flash-ROM (include code & data area) being program or erasing: 0=writing protect, 1=enable
                // program and erase
#define bDATA_WE                                                                                                       \
    0x04        // enable Data-Flash (flash-ROM data area) being program or erasing: 0=writing protect, 1=enable program
                // and erase
#define bXIR_XSFR                                                                                                      \
    0x02        // force MOVX_@R0/@R1 only for xSFR area: 0=MOVX_@R0/@R1 for standard xdata area inclde xRAM&xBUS&xSFR,
                // 1=MOVX_@R0/@R1 for xSFR only
#define bWDOG_EN                                                                                                       \
    0x01        // enable watch-dog reset if watch-dog timer overflow: 0=as timer only, 1=enable reset if timer overflow

/* Clock and Sleep and Power Registers */
SFR(PCON, 0x87);        // power control and reset flag
#define SMOD                                                                                                           \
    0x80        // baud rate selection for UART0 mode 1/2/3: 0=slow(Fsys/128 @mode2, TF1/32 @mode1/3, no effect for
                // TF2),
...
SFR(PLL_CFG, 0xB2);        // PLL clock config: lower 5 bits for PLL clock Fpll, upper 3 bits for USB 4x clock Fusb4x,
                           // Write@SafeMode
...
#define bOSC_EN_INT                                                                                                    \
    0x80        // internal oscillator enable and original clock selection: 1=enable & select internal clock, 0=disable
                // & select external clock
#define bOSC_EN_XT                                                                                                     \
    0x40        // external oscillator enable, need quartz crystal or ceramic resonator between X1 and X2 pins
#define bWDOG_IF_TO                                                                                                    \
    0x20        // ReadOnly: watch-dog timer overflow interrupt flag, cleared by reload watch-dog count or auto cleared
                // when MCU enter interrupt routine
...
SBIT(E_DIS, 0xA8, 6);        // disable global interrupts, intend to inhibit interrupt during some flash-ROM operation:
                             // 0=enable if EA=1, 1=disable
...
#define ROM_CMD_PROG                                                                                                   \
    0x9A        // WriteOnly: flash-ROM word program operation command, for changing some ROM bit of a word from 1 to 0
#define ROM_CMD_ERASE                                                                                                  \
    0xA6        // WriteOnly: flash-ROM sector erase operation command, for changing all ROM bit of 1KBytes from 0 to 1
// sfr ROM_STATUS      = 0x86;         // ReadOnly: flash-ROM status
#define ROM_STATUS ROM_CTRL
#define bROM_ADDR_OK                                                                                                   \
    0x40        // ReadOnly: flash-ROM operation address valid flag, can be reviewed before or after operation:
                // 0=invalid parameter, 1=address valid
#define bROM_CMD_ERR                                                                                                   \
    0x02        // ReadOnly: flash-ROM operation command error flag: 0=command accepted, 1=unknown command
...
//   1 1 0:  open-drain output without pullup, support input, just driving high level strongly for 2 clocks if turning
//   output level from low to high 1 0 1:  quasi-bidirectional (simulated 8051 mode), open-drain output with pullup
//   resistance 1 1 1:  quasi-bidirectional (standard 8051 mode), open-drain output with pullup resistance, just driving
...
#define bXBUS_EN                                                                                                       \
    0x08        // xBUS function enable: 0=disable, 1=enable port 0 used as data bus and P3.6/P3.7 used as bus
                // writing/reading strobe during accessing xBUS
#define bXBUS_CS_OE                                                                                                    \
    0x04        // xBUS chip-selection output enable: 0=disable output,
                //   1=output CS0 (chip-selection 0#, low action) at P3.4, output inversion of address_15 (be equal to
                //   CS1, low action) at P3.3 if ALE disabled
#define bXBUS_AH_OE                                                                                                    \
    0x02        // xBUS high address output enable: 0=disable output, 1=output address_8~15 at P2.0~P2.7 during
                // accessing xBUS by MOVX_@DPTR instruction
#define bXBUS_AL_OE                                                                                                    \
    0x01        // xBUS low address output enable: 0=multiplex low address and data bus during accessing xBUS, 1=output
                // address_0~7 at P4.0~P4.5 & P3.5 & P2.7 directly
...
#define bT1_GATE                                                                                                       \
    0x80        // gate control of timer1: 0=timer1 run enable while TR1=1, 1=timer1 run enable while P3.3 (INT1) pin is
                // high and TR1=1
#define bT1_CT                                                                                                         \
    0x40        // counter or timer mode selection for timer1: 0=timer, use internal clock, 1=counter, use P3.5 (T1) pin
                // falling edge as clock
...
#define bT0_GATE                                                                                                       \
    0x08        // gate control of timer0: 0=timer0 run enable while TR0=1, 1=timer0 run enable while P3.2 (INT0) pin is
                // high and TR0=1
#define bT0_CT                                                                                                         \
    0x04        // counter or timer mode selection for timer0: 0=timer, use internal clock, 1=counter, use P3.4 (T0) pin
                // falling edge as clock
...
//   11: mode 3, TL0 is 8-bit timer or counter controlled by standard timer0 bits, TH0 is 8-bit timer using TF1 and
//   controlled by TR1, timer1 run enable if it is not mode 3
...
SBIT(
    TI,
    0x98,
    1);        // transmit interrupt flag, set by hardware after completion of a serial transmittal, need software clear
SBIT(RI,
     0x98,
     0);        // receive interrupt flag, set by hardware after completion of a serial receiving, need software clear
SFR(SBUF, 0x99);        // UART0 data buffer: reading for receiving, writing for transmittal

/*  Timer2/Capture2 Registers  */
SFR(T2CON, 0xC8);          // timer 2 control
SBIT(TF2, 0xC8, 7);        // timer2 overflow & interrupt flag, need software clear, the flag will not be set when
                           // either RCLK=1 or TCLK=1
SBIT(CAP1F,
     0xC8,
     7);        // timer2 capture 1 interrupt flag, set by T2 edge trigger if bT2_CAP1_EN=1, need software clear
...
SBIT(C_T2, 0xC8, 1);         // timer2 clock source selection: 0=timer base internal clock, 1=external edge counter base
                             // T2 falling edge
SBIT(CP_RL2, 0xC8, 0);        // timer2 function selection (force 0 if RCLK=1 or TCLK=1): 0=timer and auto reload if
                              // count overflow or T2EX edge, 1=capture by T2EX edge
SFR(T2MOD, 0xC9);             // timer 2 mode and timer 0/1/2 clock mode
#define bTMR_CLK                                                                                                       \
    0x80        // fastest internal clock mode for timer 0/1/2 under faster clock mode: 0=use divided clock, 1=use
                // original Fsys as clock without dividing
#define bT2_CLK                                                                                                        \
    0x40        // timer2 internal clock frequency selection: 0=standard clock, Fsys/12 for timer mode, Fsys/4 for UART0
                // clock mode,
                //   1=faster clock, Fsys/4 @bTMR_CLK=0 or Fsys @bTMR_CLK=1 for timer mode, Fsys/2 @bTMR_CLK=0 or Fsys
                //   @bTMR_CLK=1 for UART0 clock mode
#define bT1_CLK                                                                                                        \
    0x20        // timer1 internal clock frequency selection: 0=standard clock, Fsys/12, 1=faster clock, Fsys/4 if
                // bTMR_CLK=0 or Fsys if bTMR_CLK=1
#define bT0_CLK                                                                                                        \
    0x10        // timer0 internal clock frequency selection: 0=standard clock, Fsys/12, 1=faster clock, Fsys/4 if
                // bTMR_CLK=0 or Fsys if bTMR_CLK=1
...
#define T2OE                                                                                                           \
    0x02        // enable timer2 generated clock output: 0=disable output, 1=enable clock output at T2 pin, frequency =
                // TF2/2
#define bT2_CAP1_EN                                                                                                    \
    0x01        // enable T2 trigger function for capture 1 of timer2 if RCLK=0 & TCLK=0 & CP_RL2=1 & C_T2=0 & T2OE=0
...
#define bT3_IE_END                                                                                                     \
    0x80        // enable interrupt for capture mode count timeout (exceed end value) or PWM mode cycle end
...
#define bT3_EN_CK_SE                                                                                                   \
    0x01        // enable to accessing divisor setting register, else enable to accessing current count register
...
#define bT3_IF_FIFO_REQ                                                                                                \
    0x20        // interrupt flag for request FIFO data (capture mode FIFO >=4 or PWM mode FIFO <=3), write 1 to clear
#define bT3_IF_ACT                                                                                                     \
    0x10        // interrupt flag for capture mode input action or PWM mode trigger if bT3_IE_ACT=1, write 1 to clear or
                // accessing FIFO to clear
#define bT3_IF_END                                                                                                     \
    0x10        // interrupt flag for capture mode count timeout (exceed end value) or PWM mode cycle end if
                // bT3_IE_ACT=0, write 1 to clear
...
#define bT3_PWM_POLAR                                                                                                  \
    0x20        // timer3 PWM output polarity: 0=default low and high action, 1=default high and low action
...
#define bPWM2_POLAR                                                                                                    \
    0x40        // PWM2 output polarity if bPWM_MOD_MFM=0: 0=default low and high action, 1=default high and low action
...
#define bPWM_IF_END                                                                                                    \
    0x10        // interrupt flag for cycle end, write 1 to clear or write PWM_CYCLE or load new data to clear
...
#define bMFM_BIT_CNT2                                                                                                  \
    0x04        // ReadOnly: MFM encode bit count status if bPWM_MOD_MFM=1: 0=lower 4 bits, 1=upper 4 bits
...
SBIT(S0_IF_OV,
     0xF8,
     6);        // interrupt flag for slave mode FIFO overflow, direct bit address clear or write 1 to clear
SBIT(S0_IF_FIRST,
     0xF8,
     5);        // interrupt flag for first byte received, direct bit address clear or write 1 to clear
SBIT(S0_IF_BYTE, 0xF8, 4);        // interrupt flag for a byte data exchanged, direct bit address clear or write 1 to
                                  // clear or accessing FIFO to clear if bS0_AUTO_IF=1
...
#define bS0_MISO_OE                                                                                                    \
    0x80        // SPI0 MISO output enable, this bit need not enable for slave mode, because MISO automatic output
                // enable if chip selected
...
#define bS1_IF_BYTE                                                                                                    \
    0x10                     // interrupt flag for a byte data exchanged, write 1 to clear or accessing FIFO to clear if
                             // bS1_AUTO_IF=1
...
#define bADC_CHANN_ID                                                                                                  \
    0x04        // ReadOnly: current channel ID for channel automatic switch mode: 0=AIN0 or AIN6, 1=AIN1 or AIN4 or
                // AIN7
#define bADC_DATA_OK                                                                                                   \
    0x04        // ReadOnly: ADC end and data ready flag for channel manual selection mode: 0=data not ready, 1=data
                // ready
...
#define MASK_ADC_CYCLE                                                                                                 \
    0x0F        // bit mask of ADC cycle (ADC clock number): 0=manual sample, others=set cycle number for automatic
                // sample
...
#define bADC_AIN7_CHK_EN                                                                                               \
    0x01                     // control AIN7 level check module power: 0=shut down, 1=enable power for AIN7 level check
...
#define MASK_ADC_AIN7_DLY                                                                                              \
    0x03        // bit mask for AIN7 check delay control: 01=longest, 10=longer, 11=shorter, 00=shortest (no delay)
...
#define bUEP_AUTO_TOG                                                                                                  \
    0x10        // enable automatic toggle after successful transfer completion on endpoint 1/2/3: 0=manual toggle,
                // 1=automatic toggle
...
SBIT(UIF_HST_SOF,
     0xD8,
     3);        // host SOF timer interrupt flag for USB host, direct bit address clear or write 1 to clear
SBIT(UIF_SUSPEND,
     0xD8,
     2);        // USB suspend or resume event interrupt flag, direct bit address clear or write 1 to clear
SBIT(UIF_TRANSFER,
     0xD8,
     1);        // USB transfer completion interrupt flag, direct bit address clear or write 1 to clear
SBIT(UIF_DETECT,
     0xD8,
     0);        // device detected event interrupt flag for USB host mode, direct bit address clear or write 1 to clear
SBIT(UIF_BUS_RST,
     0xD8,
     0);        // bus reset event interrupt flag for USB device mode, direct bit address clear or write 1 to clear
...
#define MASK_UIS_H_RES                                                                                                 \
    0x0F        // ReadOnly: bit mask of current transfer handshake response for USB host mode: 0000=no response, time
                // out from device, others=handshake response PID received
...
#define bUC_INT_BUSY                                                                                                   \
    0x08        // enable automatic responding busy for device mode or automatic pause for host mode during interrupt
                // flag UIF_TRANSFER valid
...
#define bUH_PORT_EN                                                                                                    \
    0x01        // enable USB hub port: 0=disable, 1=enable port, automatic disabled if USB device detached
SFR(UHUB1_CTRL, 0xE5);        // USB hub1 control
#define bUH1_DISABLE                                                                                                   \
    0x80        // disable USB hub1 pin: 0=enable hub1 and using HP/HM pin, 1=disable hub1 and releasing HP/HM pin
...
#define bUH_R_AUTO_TOG                                                                                                 \
    0x10        // enable automatic toggle after successful transfer completion: 0=manual toggle, 1=automatic toggle
#define bUH_R_RES                                                                                                      \
    0x04        // prepared handshake response type for host receiving (IN): 0=ACK (ready), 1=no response, time out to
                // device, for isochronous transactions
// sfr UH_EP_PID       = 0xD5;         // host endpoint and token PID, lower 4 bits for endpoint number, upper 4 bits
// for token PID
...
#define bUH_T_AUTO_TOG                                                                                                 \
    0x10        // enable automatic toggle after successful transfer completion: 0=manual toggle, 1=automatic toggle
#define bUH_T_RES                                                                                                      \
    0x01        // expected handshake response type for host transmittal (SETUP/OUT): 0=ACK (ready), 1=no response, time
                // out from device, for isochronous transactions
...
//   1 1 0:  64 bytes buffer for receiving (OUT endpoint) + 64 bytes buffer for transmittal (IN endpoint),
//   total=128bytes 1 1 1:  dual 64 bytes buffer by bUEP_R_TOG selection for receiving (OUT endpoint) + dual 64 bytes
//   buffer by bUEP_T_TOG selection for transmittal (IN endpoint), total=256bytes
...
//   1 0:  single 64 bytes buffer for endpoint 0 receiving & transmittal (OUT & IN endpoint) + 64 bytes buffer for
//   endpoint 4 receiving (OUT endpoint), total=128bytes 0 1:  single 64 bytes buffer for endpoint 0 receiving &
//   transmittal (OUT & IN endpoint) + 64 bytes buffer for endpoint 4 transmittal (IN endpoint), total=128bytes 1 1:
//   single 64 bytes buffer for endpoint 0 receiving & transmittal (OUT & IN endpoint)
//           + 64 bytes buffer for endpoint 4 receiving (OUT endpoint) + 64 bytes buffer for endpoint 4 transmittal (IN
//           endpoint), total=192bytes
#ifndef NO_XSFR_DEFINE
...
// unsigned short volatile xdata UEP0_DMA    _at_ 0x2448;   // endpoint 0&4 buffer start address, must even address,
// big-endian
#ifndef NO_XSFR_DEFINE
...
// unsigned short volatile xdata UEP1_DMA    _at_ 0x244A;   // endpoint 1 buffer start address, must even address,
// big-endian
#ifndef NO_XSFR_DEFINE
...
// unsigned short volatile __xdata UEP2_DMA    __at 0x244C;   // endpoint 2 buffer start address, must even address,
// big-endian
#ifndef NO_XSFR_DEFINE
...
// unsigned short volatile __xdata UEP3_DMA    __at 0x244E;   // endpoint 3 buffer start address, must even address,
// big-endian
#ifndef NO_XSFR_DEFINE
...
#endif
// unsigned short volatile xdata LED_DMA     _at_ 0x2884;   // DMA address, must even address, big-endian, automatic
...
#endif
// unsigned short volatile xdata LED_DMA_X   _at_ 0x2888;   // aux buffer DMA address, must even address, big-endian,
