...
__code unsigned char GetDeviceDescriptorRequest[] =
    {USB_REQ_TYP_IN, USB_GET_DESCRIPTOR, 0, USB_DESCR_TYP_DEVICE, 0, 0, sizeof(USB_DEV_DESCR), 0};
__code unsigned char GetConfigurationDescriptorRequest[] =
    {USB_REQ_TYP_IN, USB_GET_DESCRIPTOR, 0, USB_DESCR_TYP_CONFIG, 0, 0, sizeof(USB_DEV_DESCR), 0};
__code unsigned char GetInterfaceDescriptorRequest[] = {USB_REQ_TYP_IN | USB_REQ_RECIP_INTERF,
                                                        USB_GET_DESCRIPTOR,
                                                        0,
...
__code unsigned char GetDeviceStringRequest[] =
    {USB_REQ_TYP_IN, USB_GET_DESCRIPTOR, 2, 3, 9, 4, 2, 4};        // todo change language
...
__code unsigned char SetHIDIdleRequest[] =
    {USB_REQ_TYP_CLASS | USB_REQ_RECIP_INTERF, HID_SET_IDLE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
__code unsigned char GetHIDReport[] = {USB_REQ_TYP_IN | USB_REQ_RECIP_INTERF,
                                       USB_GET_DESCRIPTOR,
                                       0x00,
...
char convertStringDescriptor(unsigned char __xdata * usbBuffer,
                             unsigned char __xdata * strBuffer,
...
void pollHIDdevice(void)
{
    ...
    for (hiddevice = 0; hiddevice < MAX_HID_DEVICES; hiddevice++)
    {
        if (HIDdevice[hiddevice].connected)
        {
            ...
            s = hostTransfer(USB_PID_IN << 4 | HIDdevice[hiddevice].endPoint & 0x7F,
                             HIDdevice[hiddevice].endPoint & 0x80 ? bUH_R_TOG | bUH_T_TOG : 0,
                             0);
            if (s == ERR_SUCCESS)
            {
                ...
                {
                    LED = !LED;
                    // DEBUG_OUT("HID %lu, %i data %i : ", HIDdevice[hiddevice].type, hiddevice,
                    // HIDdevice[hiddevice].endPoint & 0x7F);
                    sendHidPollMSG(MSG_TYPE_DEVICE_POLL,
                                   len,
                                   HIDdevice[hiddevice].type,
                                   hiddevice,
                                   HIDdevice[hiddevice].endPoint & 0x7F,
                                   RxBuffer,
...
unsigned char getHIDDeviceReport(unsigned char CurrentDevive)
{
    ...
    DEBUG_OUT("\n");
    sendProtocolMSG(MSG_TYPE_HID_INFO,
                    len,
                    CurrentDevive,
                    HIDdevice[CurrentDevive].interface,
...
unsigned char initializeRootHubConnection(unsigned char rootHubIndex)
{
    ...
    for (retry = 0; retry < 10; retry++)        // todo test fewer retries
    {
        ...
        if (s == ERR_SUCCESS)
        {
            ...
            addr =
                rootHubIndex +
                ((PUSB_SETUP_REQ)SetUSBAddressRequest)->wValueL;        // todo wValue always 2.. does another id work?
            ...
            if (s == ERR_SUCCESS)
            {
                ...
                s                                   = getDeviceString();
                {
                    ...
                    if (s == ERR_SUCCESS)
                    {
                        sendProtocolMSG(MSG_TYPE_DEVICE_INFO,
                                        (receiveDataBuffer[2] + (receiveDataBuffer[3] << 8)),
                                        addr,
                                        rootHubIndex + 1,
                                        0xAA,
                                        receiveDataBuffer);
