...
void pinMode(unsigned char port, unsigned char pin, unsigned char mode)
{
    ...
    switch (mode)
    {
        ...
        case PIN_MODE_OUTPUT_OPEN_DRAIN:        // Open drain output, no pull-up, support input
            ...
            *pu[port] &= ~(1 << pin);
            break;
        case PIN_MODE_OUTPUT_OPEN_DRAIN_2CLK:        // Open-drain output, no pull-up, only drives 2 clocks high when
                                                     // the transition output goes from low to high
            PORT_CFG |= (bP0_OC << port);
            *dir[port] |= 1 << pin;
            *pu[port] &= ~(1 << pin);
            break;
        case PIN_MODE_INPUT_OUTPUT_PULLUP:        // Weakly bidirectional (standard 51 mode), open drain output, with
                                                  // pull-up
            PORT_CFG |= (bP0_OC << port);
            *dir[port] &= ~(1 << pin);
            *pu[port] |= 1 << pin;
            break;
        case PIN_MODE_INPUT_OUTPUT_PULLUP_2CLK:        // Quasi-bidirectional (standard 51 mode), open-drain output,
                                                       // with pull-up, when the transition output is low to high, only
                                                       // drives 2 clocks high
            PORT_CFG |= (bP0_OC << port);
            *dir[port] |= 1 << pin;
            *pu[port] |= 1 << pin;
            break;
